...
def process_grib_for_variable(args):
    ...
    #file_date = datetime.strptime(grib_file.split('.')[1], '%Y%m%d%H')
    #year = file_date.year
    ...
    hdr_name = narr.get_internal_file(variable, 'hdr')
    grb_name = narr.get_internal_file(variable, 'grb')
    ...
    # exist
    #base_aux_dir = System.get_base_aux_dir()
...
def download_file_list(grib_file_list, block_size):
    ...
    session = Web.Session(block_size=block_size)
...
class Noaa(object):
    ...
    def get_external_url():
        return 'http://ftp.cpc.ncep.noaa.gov/wd51we/NARR_archive/'

    @classmethod
    def download_file(cls, filename):
        logger = logging.getLogger(__name__)
        logger.info('Retrieving {0}'.format(filename))
        cls.get_session().http_transfer_file(cls.get_external_url()+filename,
                                             filename)
    ...
    def get_list_of_external_data():
        ...
        response = requests.get(Noaa.get_external_url())
    ...
    def get_session(cls):
        if(cls.session is None):
            ...
            # Log in
            cls.session.login(login_url, login_data)
        return cls.session

    @classmethod
    def get_dict_of_date_modified(cls):
        if(cls.mtime_by_name is None):
            data_list = Noaa.get_list_of_external_data()
            cls.mtime_by_name = {}
...
class NarrData(object):
    # ARCH_FMT.format(variable, year, month, day, hour*100,'hdr')
    ARCH_FMT = 'NARR_3D.{0}.{1:04}{2:02}{3:02}.{4:04}.{5}'
    # DIR_FMT.format(base_aux_dir, year, month, day)
    DIR_FMT = '{0}/{1:0>4}/{2:0>2}/{3:0>2}'
    # NOAA_FMT.format(year, month, day, hour)
    NOAA_FMT = 'rcdas.{0:04}{1:02}{2:02}{3:02}.awip32.merged'
    ...
    def get_internal_drectory(self):
        return NarrData.DIR_FMT.format(self.get_base_aux_dir(),
                                       self.dt.year, self.dt.month,
                                       self.dt.day)

    def get_internal_name(self, variable, ext):
        return NarrData.ARCH_FMT.format(variable, self.dt.year, self.dt.month,
                                        self.dt.day, self.dt.hour*100, ext)

    def get_internal_file(self, variable, ext):
    ...
    def get_internal_last_modified(self, variable='HGT', ext='hdr'):
        try:
            filename = self.get_internal_file(variable, ext)
    ...
    def get_external_name(self):
        return NarrData.NOAA_FMT.format(self.dt.year, self.dt.month,
                                        self.dt.day, self.dt.hour)

    def get_external_last_modified(self):
        try:
            return Noaa.get_dict_of_date_modified()[self.get_external_name()]
        except KeyError:
            raise NarrData.FileMissing
    ...
    def download_grib(self):
        Noaa.download_file(self.get_external_name())

    def extract_var_from_grib(self):
        for var in NarrData.variables:
            process_grib_for_variable((var, self.get_external_name()))

    def clean_downloaded_grib(self):
        if os.path.exists(self.get_external_name()):
            os.unlink(self.get_external_name())

    @staticmethod
    def from_external_name(external_name):
        date_measured = datetime.strftime(external_name.split('.')[1],
                                          '%Y%m%d%H')
...
def update(data_to_updated):
    for data in data_to_updated:
        data.download_grib()
...
def report(data_to_report):
    ...
    # print('\n'.join(Noaa.get_list_of_external_data()))
    # print(Noaa.get_dict_of_date_modified())

    report = []
    for data in data_to_report:
        try:
            line = [data.dt.isoformat(),
                    data.get_internal_last_modified().isoformat(),
                    data.get_external_last_modified().isoformat()]
...
def main():
    ...
    data_to_updated = filter(lambda x: x.need_to_download(), data)
    if True:
        print('Measured,UpdatedLocally,UpdatedOnline')
        print(report(data_to_updated))
    if True:
        update(data_to_updated)
